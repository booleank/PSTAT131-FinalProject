...
## Logistic Regression
```{r }
# Logistic regression model using first 8 PCs
...
# errors
error_glm <- table(y = pc_data$winner, y_hat_glm)
error_glm

# total misclassification rate
tot_misclass_glm <- 1 - sum(diag(error_glm))/nrow(pc_data)
tot_misclass_glm
```
...
cv_out <- tibble(k = seq_range(5:100, n = 20, pretty = T)) %>%
  mutate(loocv_preds = map(k, ~knn.cv(pc_data[-1], y, .x)),
         class = map(k, ~ y)) %>%
...
# re-train
y_hat_knn <- knn.cv(train = pc_data[-1], cl = y, k = best_k)

# misclassifications
error_knn <- table(y, y_hat_knn)
error_knn

# total misclassification rate
tot_misclass_knn <- 1 - sum(diag(error_knn))/nrow(pc_data)
tot_misclass_knn
```
...
## Establish Testing/Training Sets... (haven't used yet)
```{r}
# set rng
set.seed(20121)
...
misclass_rf
```
Random forest shows lowest misclass

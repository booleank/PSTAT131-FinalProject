...
In each model above we predict the probability of win for Donald Trump, $p_i=P(y_i='Don'),\,\,\forall i$. Then we consider a threshold $\alpha \in (0,1)$. If the predicted probability from model M, $\hat{p}_i^M > \alpha$, then we predict that that Trump won, i.e. $\hat{y}_i^M='Don'$, otherwise $\hat{y}_i^M='Hill'$. Here $\hat{y}_i^M$ is the predicted response for $i^{th}$ county from the model M above.

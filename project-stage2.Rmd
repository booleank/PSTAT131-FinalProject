...
In each model above we predict the probability of win for Donald Trump, $p_i=P(y_i=1),\,\,\forall i$. Then we consider a threshold $\alpha \in (0,1)$. If the predicted $p_i > \alpha$, then we predict that that Trump won, i.e. $\hat{y_i}^M(\alpha)='Don'$, otherwise $\hat{y_i}^M(\alpha)='Hill'$. Here $\hat{y}_i^M(\alpha)$ is the predicted response for $i^{th}$ county from the model M above for a threshold level $\alpha$.

Then we compute the misclassification for $i^{th}$ county: 
$$z_i^M(\alpha)=\left\{\begin{array}{lll}
0 & , & y_i=\hat{y}_i^M(\alpha) \\
1 & , &  y_i \ne \hat{y}_i^M(\alpha) \\
\end{array}\right.$$
...
However which model thurned out the winner optimize for $\alpha$ over a grid of $(0,1)$.
...
For categorical response, and and covariates being Principal components are in euclidean space of p-dimension, we follow similar steps as earlier models and train the model on 70%
data and test the performance on the rest. The total misclassification error rates turning out to be 0.0837 for the LDA and 0.08913043 for the QDA. Hence LDA performs slightly better. But their performances are not at par with the random forest above.

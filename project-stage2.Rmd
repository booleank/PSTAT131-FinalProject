...
In each model above we predict $p_i=P(y_i=1),\,\,\forall i$, the probability of win for Donald Trump in $i^{th}$ county. Then we consider a threshold $\alpha \in (0,1)$. If the predicted $p_i > \alpha$, then we predict that that Trump won, i.e. $\hat{y_i}=1$, otherwise $\hat{y_i}=0$. Here $\hat{y}_i$ is the predicted response for $i^{th}$ county from any particular model above. 
...
To optimize over the tuning parameter $\alpha$, we consider an equidistant grid over $(0,1)$, and 

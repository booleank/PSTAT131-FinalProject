...
In each model above we predict the probability of win for Donald Trump, $p_i=P(y_i=1),\,\,\forall i$. Then we consider a threshold $\alpha \in (0,1)$. Let $\hat{p}_i^M$ be the predicted probability for win of Donald trump for $i^{th}$ county from model M. If $\hat{p}_i^M > \alpha$, then we predict that that Trump won, i.e. $\hat{y_i}^M=1$, otherwise $\hat{y_i}^M=0$. Here $\hat{y}_i^M$ is the predicted response for $i^{th}$ county from the model M above.

...
**Task**:
Based on the given data we try to predict the winner of general election 2016 in each state and check how the model prediction agrees with the results in each state. First we want to choose an optimal set of covariates to predict the response. For that we use Principal Component Analysis (PCA). Using scree/cumulative variability plot we determine at which PC the explained variability exceeds 80%, then use them as covariates for the above models we want to compare. We also check from the PC loadings plot to check which covariates weighs more than others in the PCs. That way we get some idea which covariates are more important. Then we want to classify the winning states for the winning candidate, we are looking for the best classification method among the following:
...
After predicting the probabilities we compute the optimum threshold from using Youden's statistic and classify the winning or losing states accordingly. Then we compare the FPR and FNR for each model and come up with 3 best models interms of performance and assumptions.

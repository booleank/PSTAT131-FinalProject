...
The variables with the highest loading magnitudes drive variation in the data. These variables include Child Poverty, Commuter Rate, Employment Rate, Income Per Capita, Poverty, Private Work, Unemployment, and Work at Home. General information about income and employment, such as Child Poverty, Income Per Capita, Employment and Unemployment, corresponds to PC1. Specific professional information, such as Commuter Rate, Work at Home, and Private Work corresponds to PC2. 
...
census_pc %>%
  ...
  geom_text(aes(label = ifelse((PC2<(-6)&PC1<(-5)), as.character(County), ''), hjust = 0, vjust = 0))
```
...
Kusilvak Census Area is a clear outlier, since it has a significantly lower PC2 value than any other county. Oglala Lakota and Todd Counties also have exceptionally low combinations of PC1 and PC2 loading values. Each of these counties have majority Native American populations and exceptionally low income per capita. In fact, they are the three poorest counties in the 50 states according to census data. 

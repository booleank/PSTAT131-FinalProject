...
loadings[,1:2] %>% 
  as.data.frame() %>%
  mutate(sum = abs(V1+V2)) %>%
  select(sum) %>%
  mutate(var = colnames(x_mx)) %>%
  arrange(desc(sum))
```
 
The variables with the highest loading magnitudes drive variation in the data. These variables include White, Poverty, Minority, PrivateWork, ChildPoverty, Employed, and Unemployment. This indicates that differences among counties' racial demographics, employment rates, private industry work rates, and poverty rates are the main drivers of election outcome variability. 
